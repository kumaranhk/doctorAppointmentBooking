...
const Navbar = () => {
  ...
  if (user?.role === "patient") {
    menu = ["/", "/doctors", "/about", "/contact"];
  } else if (user?.role === "admin") {
    menu = ["/admin/dashboard","/admin/doctors", "/admin/appointments", "/admin/patients"];
  }
  else if (user?.role === 'doctor'){
    menu = ['/doctor/appointments'];
  }
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <Avatar alt="Profile Name" src={assets.defaultPic} />
        </IconButton>
        ...
        >
          <List sx={{ width: "250px" }}>
            {menu.map((path, index) => (
              ...
              >
                <ListItemText
                  primary={user.role === 'patient' ? path === "/" ? "HOME" : path.slice(1).toUpperCase() : path.split('/')[2].toUpperCase()}
                />
              </ListItem>
            ))}
            {!user ? (
